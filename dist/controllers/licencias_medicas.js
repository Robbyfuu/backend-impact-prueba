...
export const getLicenciaMedicaTrabajador = async (req, res) => {
...
export const getLicenciasMedicasTerminadas = async (req, res) => {
    try {
        const licenciasMedicas = await LicenciaMedica.findAll({
            where: {
...
export const getLicenciasMedicasActivas = async (req, res) => {
    try {
        const licenciasMedicas = await LicenciaMedica.findAll({
            where: {
...
export const putDesactivarLicenciaMedica = async (req, res) => {
    try {
        const licenciaMedica = await db.query(`UPDATE licencias_medicas SET estado_licencia = FALSE WHERE fecha_fin < CURRENT_DATE`);
        if (!licenciaMedica) {
            return res.status(404).json({
...
export const postLicenciasMedicasMasiva = async (req, res) => {
    const { body } = req;
    try {
