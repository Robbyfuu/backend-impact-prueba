...
export const getSueldoTrabajador = async (req, res) => {
    const { id } = req.params;
...
export const postSueldo = async (req, res) => {
    const { body } = req;
    try {
        ...
        if (existeFicha) {
            return res.status(400).json({
                msg: `Trabajador ${body.ficha} ya registra sueldo para el mes` +
                    body.fecha_mes,
                ok: false,
            });
        }
...
export const putSueldo = async (req, res) => {
    const { id } = req.params;
    const { body } = req;
    try {
...
export const getUF = async (req, res) => {
...
export const getSalary3monthsAgo = async (req, res) => {
    const { id } = req.params;
    const fecha = new Date();
    fecha.setHours(0, 0, 0, 0);
    const sueldos = await db.query(`SELECT * from sueldos_mensuales WHERE fecha_mes >= Date_sub( DATE_SUB(CURRENT_DATE, INTERVAL 3 MONTH),INTERVAL day(CURRENT_DATE)-1 DAY) && ficha =${id}`);
    logger.info(sueldos);
    res.json({ ok: true, sueldos: sueldos[0] });
};
export const getSalarylastMonth = async (req, res) => {
    const { id } = req.params;
    const fecha = new Date();
    fecha.setHours(0, 0, 0, 0);
    const sueldos = await db.query(`SELECT * from sueldos_mensuales WHERE fecha_mes = Date_sub( DATE_SUB(CURRENT_DATE, INTERVAL 0 MONTH),INTERVAL day(CURRENT_DATE)+0 DAY) && ficha =${id}`);
    logger.info(sueldos);
    res.json({ ok: true, sueldos: sueldos[0] });
};
